...
3. **管理端**：服主自托管的 Java 应用，提供完整 CLI、本机 Web 管理界面、SQLite 元数据存储和公共 HTTP 文件服务。
...
flowchart LR
    ...
    M["服主"] -->|"本机 CLI 或 SSH 隧道"| C["CLI / Web 管理适配器"]
    C --> S["管理端应用服务"]
    H --> S
    S --> D["SQLite"]
    S --> O["哈希寻址内容目录"]
...
管理接口与玩家下载接口使用独立监听器。Web 管理端固定监听 `127.0.0.1`，远程管理必须通过 SSH 隧道访问；面向玩家的只读 HTTP 文件端点可监听公网网卡。CLI 和 Web 都直接调用同一应用服务，不向公网提供无加密的管理入口。
...
| --- | --- | --- | --- | --- |
| 普通托管 | 写入 | 覆盖 | 按发布时选择删除或放弃管理 | 保留 |
| 非托管/排除 | 不处理 | 不处理 | 不处理 | 保留 |
| 强制同步目录 | 写入 | 覆盖 | 归档旧文件 | 归档到玩家长期备份 |

标准整合包目录内未匹配排除规则的文件都是普通托管文件。目录可以只有 `mods/` 和 `config/`，此时其它玩家目录不在目标文件清单中，也不会被更新器修改。发布预览显示最终策略、增删改列表与预计下载量。

新建项目会预置六条自动排除：`.dreamingfish-bootstrap/**` 与 `DreamingFishUpdater/**` 用于保护更新器自身，`logs/**`、`crash-reports/**`、`saves/**` 与 `screenshots/**` 用于避免误发布玩家运行数据。除此之外，不按文件名、扩展名或 Minecraft 目录名称附加其它隐藏策略；`options.txt` 和 `servers.dat` 与普通文件完全相同。
...
CLI 与 WebUI 调用同一应用服务层，发布、签名和备份规则不写在命令解析或 HTTP 处理代码中。WebUI 覆盖日常发布流程，加密备份与恢复只保留在 CLI。
...
发布描述包含项目 ID、发布 ID、递增序号、创建时间、清单哈希、最低玩家端版本、更新说明、强制同步一级目录、强制同步文件、已释放路径和签名。目标文件条目至少包含规范化相对路径、SHA-256、大小和文件策略。

签名覆盖服务器实际发送的确定性字节，不由客户端重新拼装 JSON 后验证。客户端先验证签名和项目身份，再信任其中的 URL、版本和哈希。路径必须是正斜杠相对路径，并拒绝绝对路径、`..`、设备名、大小写冲突和逃逸实例目录的符号链接。

清单具有明确的格式版本。未知可选字段可以忽略；未知必要能力、过低玩家端版本或非法字段必须安全失败。目录强制同步、单文件强制同步和放弃管理分别声明 `forced-directory-sync-v1`、`forced-file-sync-v1` 和 `released-paths-v1` 必要能力，旧玩家端必须先自更新，不能静默忽略。HTTP 服务支持 `Range`、`ETag` 和合理缓存头，以便断点续传和重复对象复用，但 V1 不引入 CDN 配置。
...
桌面窗口初始尺寸为 `1180 x 680`，最小尺寸为 `960 x 560`，支持最大化、还原和双击标题栏切换。视觉验收只以参考网页的电脑端封面为准，不采用手机端主构图，也不在窄窗口中改成移动端纵向堆叠。窗口采用自定义但熟悉的最小化、最大化和关闭按钮，并保留键盘焦点、缩放和高 DPI 支持。详情抽屉可在右侧栏与完整内容区之间切换。
...
- **未验证离线启动**：明确说明无法连接更新服务器、本次未检查整合包且未执行文件验证；
...
management-cli/        完整 picocli CLI、本机 Web 管理界面与服务控制
...
已实现：协议与签名、管理 CLI、本机 WebUI、扫描预览、不可变发布、只读 HTTP、事务更新与崩溃恢复、离线许可、Java 8 Agent、启动前玩家程序验证、玩家端自更新与失败回退、桌面 JavaFX 界面、完整加密备份、Windows/Linux 管理端打包。

明确延后：公网 Web 管理、黑名单和服务器准入、反作弊、CDN、对象自动垃圾回收、玩家端旧版本自动清理、程序内 HTTPS。延后项不是 V1 正常发布与更新闭环的依赖。

## 14. 验收底线

- 断网时优先验证最近一次已验证安装；没有签名基线时允许明确标记的未验证启动，但任何完整性错误仍必须阻断；
