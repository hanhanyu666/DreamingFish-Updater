...
首次启动询问玩家是否修改玩家端安装目录，默认按钮应明确推荐保留在当前实例内。玩家选择其他目录时，当前程序先复制并校验完整的新程序目录，再原子更新实例内绑定文件的本机安装路径字段；项目 ID、公共地址和公钥等信任字段保持不变，新位置从下次启动生效。Agent 和项目绑定文件始终留在实例目录，因此不需要修改 PCL2、HMCL 或 Prism Launcher 的 JVM 参数。
